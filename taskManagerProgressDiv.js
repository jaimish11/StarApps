...
function createTaskProgressBar(serverListContainer, availableServerNumber=null, i=null, callback){
    var taskProgressBar = document.createElement('div');
    taskProgressBar.setAttribute("class","task-progress-bar");
    taskProgressBar.setAttribute("id","task-progress-bar-"+i);
    
    var progressInner = document.createElement('div');
    progressInner.setAttribute("class","progress-inner");

    taskProgressBar.appendChild(progressInner);

    progressInner.style.animationDuration = TASK_DURATION;
...
function runPendingTasks(addedServerID){
    ...
    if(pendingTasks>0){
        ...
        createTaskProgressBar(serverListContainer, addedServerID, null, function(){
            toggleServer(addedServerID, false);
            var taskProgressBar = document.querySelector('#task-progress-bar-'+addedServerID);
            debugger;
            taskProgressBar.parentNode.removeChild(taskProgressBar);
            if(pendingTasks==0){
                //Always keep 1 server active - delete 2 to N servers
...
function createTask(serverListContainer, noOfTasks, addedServerID){
    ...
    for(let i=1;i<=noOfTasks;i++){  
        ...
        if(Number.isInteger(availableServerNumber)){
            ...
            createTaskProgressBar(serverListContainer, availableServerNumber, i, function(){
                pendingTasks--;
                toggleServer(i, false);
                console.log(serverStatusList);
                var taskProgressBar = document.querySelector('#task-progress-bar-'+i);
                debugger;
                taskProgressBar.parentNode.removeChild(taskProgressBar);
